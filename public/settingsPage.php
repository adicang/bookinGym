...
<section class="container-fluid padding transferPadding">
<a href="settingsPage.php" class="btn btn-primary text-center sign_up toRight">לחץ כאן לעריכה/מחיקת מודעה</a>
</section>
<div class="clear"></div>
  ...
  <section class="container-fluid padding">
	  ...
	  if(isset ($_POST['deletePost'])){
		  ...
		  $key=$_POST['toBeDelete'];
		 //check if the record exist to delete or not
		 $check="select * from cardTransfer where cardTransfer_id='$key'" or die ("not found".mysql_errno());
		 
		$result=$database->query($check);
		
		 $rows=$result->num_rows;
		 if($rows>0){
				$queryDelete="DELETE from cardTransfer where cardTransfer_id='".$key."'"
...
?>
  ...
  <div>
		<form action="" method="post">
			<td>
				<input type="checkbox" name="toBeDelete" value="<?php echo $row["cardTransfer_id"];?>" required>
			</td>			
			<td>
				<input type="submit" name="deletePost"  value="delete"class="btn btn-info">
			</td>
		</form>
	</div>
	<?php 	
